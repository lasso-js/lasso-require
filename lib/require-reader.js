...
function DEFAULT_READER_STREAM(path, optimizerContext) {
    return fs.createReadStream(path, {encoding: 'utf8'});
}

exports.stream = function stream(path, optimizerContext, readFunc) {
    if (!readFunc) {
    ...
    var result = readFunc(path, optimizerContext, callback);
    if (result == null) {
