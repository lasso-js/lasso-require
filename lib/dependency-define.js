...
function transformRequires(code, inspected, asyncBlocks, optimizerContext, callback) {
    ...
    function transformRequire(require) {
        ...
        if (!resolved.isDir && (resolved.dependencyType || !optimizerContext.dependencyRegistry.getRequireHandler(resolved.filePath, optimizerContext))) {
            if (require.range) {
    ...
    callback(null, stringTransformer.transform(inspected.code));
}
