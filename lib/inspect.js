...
exports.inspectCached = function (path, reader, getLastModified, optimizerContext, config, callback) {
    ...
    getLastModified(function(err, lastModified) {
        ...
        var inspectCache = optimizerContext.data['optimizer-require/inspect'];
        if (!inspectCache) {
            inspectCache = optimizerContext.data['optimizer-require/inspect'] = optimizerContext.cache.getCache(
                    'optimizer-require/inspect' + transformsId, // <-- Unique cache name based on the set of enabled require transforms
                    'optimizer-require/inspect');               // <-- Name of the cache configuration to use
        }
