    ...
    function doInspect() {
        ...
        return raptorPromises.makePromise()
            ...
            .then(function(src) {
                ...
                if (debugEnabled) {
                    logger.debug('Inspection of ' + path + ' found following requires: ' + inspect.requires);
                }
    ...
    optimizerContext.getFileLastModified(path, function(err, lastModified) {
        ...
        inspectCache.get(cacheKey, {
            ...
            if (!inspect.transformed) {
                ...
                if (debugEnabled) {
                    logger.debug('Cache hit. File was not tranformed');
                }
                return deferred.resolve({
                    requires: inspect.requires,
                    async: inspect.async,
            ...
            transformedCache.getReadStream(cacheKey, {
                builder: function(callback) {
                    builder(function(err, cacheEntry) {
                        ...
                        logger.warn('Cache hit for inspected source. Cache miss for transformed file.');
                        inspect = cacheEntry;
