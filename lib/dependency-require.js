...
function inspectSource(dependency, resolved, optimizerContext, config) {
    ...
    if (!optimizerContext || !optimizerContext.cache) {
        ...
        return doInspect().then(function(inspect) {
            inspect.reader = resumer().queue(inspect.code).end();
            return inspect;
        });
    }
    ...
    optimizerContext.getFileLastModified(path, function(err, lastModified) {
        ...
        var inspectCache = optimizerContext.cache.getCacheByName('raptor-optimizer-require/inspect', INSPECT_CACHE_CONFIG);
        var transformedCache = optimizerContext.cache.getCacheByName('raptor-optimizer-require/transformed', TRANSFORMED_CACHE_CONFIG);
        var reader;
        ...
        function builder(callback) {
            doInspect().then(function(inspect) {
                ...
                if (debugEnabled) {
                    logger.debug('Caching inspection of ' + path + ': ' + JSON.stringify(cachedValue));
                }
        ...
        inspectCache.get(cacheKey, {
            ...
            if (debugEnabled) {
                    logger.debug('Caching store returned for ' + path + ': ' + JSON.stringify(inspect));
                }

            if (reader !== undefined) {
                // NOTE: reader variable is assigned by the builder function
                // before this callback was invoked
                transformedCache.put(cacheKey, reader, {
                    lastModified: lastModified
                });
            ...
            if (!inspect.transformed) {
                ...
                return deferred.resolve({
                    ...
                    async: inspect.async,
                    processGlobal: inspect.processGlobal,
                    reader: requireReader.stream(path, optimizerContext)
                });
            }
        ...
        getDependencies: function(optimizerContext) {
            ...
            return inspectSource(this, resolved, optimizerContext, config).then(function(inspect) {
                ...
                if (inspect) {
                    ...
                    if (logger.isDebugEnabled()) {
                        logger.debug('Found requires for "' + resolved.filePath + '": ' + JSON.stringify(inspect.requires));
                        logger.debug('Found async for "' + resolved.filePath + '": ' + JSON.stringify(inspect.async, null, ' '));
                    }
