...
function defaultLastModified(path, optimizerContext, callback) {
...
function inspectSource(dependency, resolved, optimizerContext, config, callback) {
    ...
    function doInspect(callback) {
        ...
        if (transforms) {
            // apply transforms
            inStream = transforms.apply(path, inStream, optimizerContext);
        } else {
            if (debugEnabled) {
    ...
    var lastModifiedFunc = dependency._lastModified || defaultLastModified;

    // determined the last modified of the source file that we are inspecting
    lastModifiedFunc(path, optimizerContext, function(err, lastModified) {
        ...
        var transformedCache = optimizerContext.data['raptor-optimizer-require/transformed'];
        if (!transformedCache) {
            transformedCache = optimizerContext.data['raptor-optimizer-require/transformed'] = optimizerContext.cache.getCache(
        ...
        function builder(callback) {
            doInspect(function(err, inspect) {
                ...
                reader = function() {
                    ...
                    return through().pause().queue(inspect.code).end();
                };
