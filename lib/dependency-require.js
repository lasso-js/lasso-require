...
var resolveInspectedRequires = require('./util/resolve').resolveInspectedRequires;
var resolveRequire = require('./util/resolve').resolveRequire;
...
function buildAsyncMeta(asyncBlocks, optimizerContext) {
...
function create(config, optimizer) {
    ...
    return {
        ...
        init: function(optimizerContext, callback) {
            ...
            if (this._resolved) {
                this.resolvedPath = this._resolved.filePath;
                return callback();
                ...
                resolveRequire(path, from, fromFile, options, function(err, resolved) {
                    if (err) {
                        return callback(err);
                    }
        ...
        getDependencies: function(optimizerContext, callback) {
            ...
            var dependencies = [];
            var deduper = new Deduper(optimizerContext, dependencies);
            ...
            function getDependenciesFromInspected(err, inspected) {
                ...
                if (inspected && inspected.asyncBlocks.length) {
                    asyncMeta = buildAsyncMeta(inspected.asyncBlocks, optimizerContext);
                }
                ...
                if (!optimizerContext.isAsyncBundlingPhase()) {
                ...
                if (main) {
                    ...
                    if (requires && requires.length) {
                        requires.forEach(function(inspectedRequire) {
                            deduper.addRequire({
                                type: 'require',
                                _resolved: inspectedRequire.resolved
                            });
                        });
                    }
                }
