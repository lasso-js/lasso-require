...
function generateTransformedFileName(filePath) {
    ...
    var file = nodePath.join(this.dir, checksum.charAt(0), checksum + extension);
            ...
            .then(function(cacheEntry) {
                ...
                return doInspect().then(function(inspect) {
                    ...
                    if (inspect.modified && inspect.code) {
                        ...
                        cacheEntry.transformedFile = transformedFile = 'transformed/' + generateTransformedFileName(path);
                        var transformedFilePath = nodePath.join(context.cache.getCacheDir(), transformedFile);
...
function create(config) {
    ...
    return {
        ...
        getDependencies: function(context) {
            ...
            var requireContext = context.attributes[module.id] || (context.attributes[module.id] = {});

            var resolved = this._resolved;
            var run = this.run === true;
            var root = this.root;

            return inspectSource(resolved, context, this._reader, config).then(function(inspect) {
                var requires;
                ...
                // Include commonjs-ready if we haven't included it yet
