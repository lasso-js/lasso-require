...
var getRequireRemapFromDir = require('./util/getRequireRemapFromDir');
    ...
    return {
        ...
        init: function(lassoContext, callback) {
            ...
            if (this._resolved) {
                ...
                var remap = getRequireRemapFromDir(nodePath.dirname(this.resolvedPath), lassoContext);
                if (remap) {
                    if (options) {
                        options.remap = remap;
                    } else {
        ...
        getDependencies: function(lassoContext, callback) {
            ...
            if (resolved && resolved.voidRemap) {
                // This is a short circuit for a module that has been
                // remapped to a void module using the "browser" field
                // in package.json
                var remapDeps = [];
                if (resolved.filePath) {
                    // This was a local module that has been remapped
                    remapDeps.push(extend({
                        type: 'commonjs-def',
                        object: true,
                ...
                if (main) {
                    ...
                    if (requires && requires.length) {
                        requires.forEach(function(inspectedRequire) {
                            ...
                            if (inspectedResolved.voidRemap) {
                                // voidRemap will be set to true if the module has been
                                // remapped to a void module using package.json:
                                //
                                // {
                                //   "browser": {
                                //     "my-server-only-module": false,
                                //     "./my-server-only-module": false
                                //   }
                                // }
                                //
                                // The remapped module can either be an installed module
                                // or a relative path to another module. They are handled
                                // slightly differently.
                                if (inspectedResolved.filePath) {
                                    // This was a local module that has been remapped
                                    deduper.addDef(extend({
                                        type: 'commonjs-def',
                                        object: true,
                                        _requireReader: function() {
                                            return lassoContext.deferredStream(function() {
                                ...
                                } else if (inspectedResolved.dep) {
                                    // This was an installed module that has been remapped
                                    deduper.addDep(extend({
                                        type: 'commonjs-dep'
                                    }, inspectedResolved.dep));
                                } else {
            ...
            if (isDir) {
                ...
                // Static JavaScript objects do not need to be inspected.
                if (requireHandler.object) {
                    return requireHandler.getLastModified(function(err, lastModified) {
                        getDependenciesFromInspected(null, {
                            reader: requireHandler.reader,
